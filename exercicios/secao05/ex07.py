...
blue_text = '\033[34m'
n1 = float(
    input(f'Informe um {red_text}número{normal_text}: ')
    .strip()
    .replace(',', '.')
)
n2 = float(
    input(f'Informe outro {blue_text}número{normal_text}: ')
    .strip()
    .replace(',', '.')
)
if n1 == n2:
    ...
    print(
        f'O número {red_text}{n1}{normal_text} é diferente de {blue_text}{n2}{normal_text}!'
    )
