...
def main() -> None:
    ...
    while len(gabarito) < 10:
        ...
        if (
            (alternativa.lower() == 'a')
            or (alternativa.lower() == 'b')
            or (alternativa.lower() == 'c')
            or (alternativa.lower() == 'd')
            or (alternativa.lower() == 'e')
        ):
    ...
    while len(alunos) < 3:
        ...
        if matricula not in matriculas:
            ...
            for j in range(10):
                resposta = str(
                    input(
                        f'Digite a resposta da questão {j + 1} do aluno {cont + 1}: '
                    )
                )
                if (
                    (resposta.lower() == 'a')
    ...
    for i in range(3):
        ...
        print(
            f'Percentual de aprovação do aluno {i + 1}: {percentual_aprovacao}%'
        )
