    ...
    for i in range(n):
        while True:
            try:
                number = atof(
                    input(
                        f'Enter the {i + 1}º number for array "{name}":\n-> '
                    )
                )
                yield number
                break
...
def main():
    ...
    formatted_array_a = ', '.join(format_string('%.1f', x) for x in array_a)
    formatted_array_b = ', '.join(format_string('%.1f', x) for x in array_b)
    result: float = 0
    for i in range(5):
